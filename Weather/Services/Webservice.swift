...
struct Webservice {
  func getDailyWeather(_ city: String, completion: @escaping ([DailyWeather]?) -> Void) {
    guard let url = URL(string: "https://api.weatherbit.io/v2.0/forecast/daily?city=\(city)&key=1524f1878a7c47fcac61ccd561b36df0") else {
      completion(nil)
      return
    }
  ...
  func getCurrentWeather(_ city: String,completion: @escaping (CurrentWeatherModel?) -> Void) {
    guard let url = URL(string: "https://api.weatherbit.io/v2.0/current?city=\(city)&key=1524f1878a7c47fcac61ccd561b36df0") else {
      completion(nil)
      return
    }
    Alamofire.request(url).responseData { (response) in
